// ==========================================
// SNAKES & LADDERS PRO - 3 PLAYER ENGINE
// ==========================================

const boardData = {
    'board1': { image: 'board1.jpg', padding: 0, snakes: { 17:7, 54:34, 62:19, 64:60, 87:36, 93:73, 95:75, 98:79 }, ladders: { 1:38, 4:14, 9:31, 21:42, 28:84, 51:67, 72:91, 80:99 } },
    'board3': { image: 'board3.jpg', padding: 3, snakes: { 16:7, 59:17, 63:19, 67:30, 87:24, 93:69, 95:75, 99:77 }, ladders: { 9:27, 18:37, 25:54, 28:51, 68:88, 79:100 } },
    'board4': { image: 'board4.jpg', padding: 16, snakes: { 27:5, 40:3, 43:18, 54:31, 66:45, 89:53, 95:77, 99:41 }, ladders: { 4:25, 13:46, 42:63, 50:69, 62:81, 74:92 } }
};

let currentBoard = boardData['board1']; 
let activePlayers = ['red', 'green', 'blue']; 
...
let timeLeft = 25;
let missedTurns = { 'red': 0, 'green': 0, 'blue': 0 };

let winnersList = [];
let totalPlayersInGame = 3;
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    Object.keys(players).forEach(color => {
        players[color].element.style.transform = `translate(-40px, 450px)`;
    });
    checkGameModeSetup();
});
...
function connectToServer() {
    if (!socket) {
        ...
        socket.on('start-online-game', (data) => {
            if (data.selectedBoard) setBoardLayout(data.selectedBoard);
...
function updatePawnPosition(playerColor) {
    ...
    if (pos === 0) {
        player.element.style.transform = `translate(-40px, 450px)`;
        return;
    }
    ...
    let centerX = cellX + (cellSize - 20) / 2; 
    let centerY = cellY + (cellSize - 26) / 2;

    let offsets = { 'red': {x: -5, y: -5}, 'green': {x: 5, y: -5}, 'blue': {x: 0, y: 5} };
...
function handlePlayerWin(playerColor) {
    ...
    if (!winnersList.includes(playerColor)) {
        ...
        if (winnersList.length >= 2 || activePlayers.length <= 1) {
            endMatchAndGoToMenu();
        } else {
...
function endMatchAndGoToMenu() {
    ...
    setTimeout(() => {
        ...
        if(activePlayers.length === 1) winMessage += `Rank 3: ${activePlayers[0].toUpperCase()}\n`;
