// ==========================================
// SNAKES & LADDERS PRO - 2 PLAYER ENGINE
// ==========================================
...
let timeLeft = 25;
let missedTurns = { 'red': 0, 'blue': 0 };

let winnersList = [];

const playersData = {
    'red': { name: "RED'S TURN", class: "red-text" },
    'blue': { name: "BLUE'S TURN", class: "blue-text" }
};
...
function connectToServer() {
    if (!socket) {
        ...
        socket.on('opponent-left', (data) => {
            clearTurnTimer();
            alert(`🏆 You win the match because the other player (${data.color.toUpperCase()}) left the game!`);
            setTimeout(() => {
                window.location.href = "index.html";
            }, 1000);
        });
...
function showMyIdentity(color) {
    ...
    let bgColor = (color === 'red') ? '#e74c3c' : '#3498db';
...
function handleTurnTimeout(color) {
    ...
    if (missedTurns[color] >= 3) {
        clearTurnTimer();
        alert(`🚨 ${color.toUpperCase()} missed 3 turns and lost!`);
        endMatchAndGoToMenu();
        return;
    }
...
function updatePawnPosition(playerColor) {
    ...
    let centerX = cellX + (cellSize - 24) / 2;
    let centerY = cellY + (cellSize - 32) / 2;
...
function handlePlayerWin(playerColor) {
    clearTurnTimer();
    alert(`🏆 WINNER! ${playerColor.toUpperCase()} won the game! 🏆`);
    endMatchAndGoToMenu();
}
