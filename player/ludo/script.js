/* =========================================
   ULTRA LUDO PRO - FULL ONLINE MULTIPLAYER ENGINE
   (Original Logic + Callback Pause)
========================================= */
...
const safeZones = [{r:6, c:1}, {r:8, c:2}, {r:1, c:8}, {r:2, c:6}, {r:8, c:13}, {r:6, c:12}, {r:13, c:6}, {r:12, c:8}];
const diceFaces = ['', '⚀', '⚁', '⚂', '⚃', '⚄', '⚅'];

// 🔥 AD WAIT LOGIC UPDATED
let lastAdTime = 0;
...
function connectToServer() {
    if (!socket) {
        ...
        socket.on('room-created', (data) => {
            window.currentRoomId = data.roomId; myAssignedColor = data.color; showMyIdentity(data.color);
            document.getElementById("room-created-display").innerText = `Room created! ID: ${data.roomId}\nWaiting for players...`;
        });
        socket.on('joined-success', (data) => {
            window.currentRoomId = data.roomId; myAssignedColor = data.color; showMyIdentity(data.color);
            alert(`Joined Room: ${data.roomId} | Color: ${data.color}`);
        });
        ...
        socket.on('remote-dice-rolled', (data) => {
            clearTurnTimer(); currentDiceValue = data.diceValue; currentPlayerIndex = data.playerIndex;
            const diceContainer = document.getElementById("dice-container");
            diceContainer.innerText = diceFaces[currentDiceValue];
            diceContainer.style.color = currentDiceValue === 6 ? "#ff2a2a" : "#111";
            gameState = 'WAITING_FOR_MOVE'; startTurnTimer(); checkAvailableMoves();
        });
        socket.on('remote-token-moved', (data) => {
            clearTurnTimer();
            let color = data.color; let tokenIndex = data.tokenIndex; let diceVal = data.diceVal;
            ...
            if (tokenObj.state === 'home' && diceVal === 6) {
                tokenObj.state = 'active'; tokenObj.pathPosition = 0; updateTokenUI(color, tokenIndex); switchTurn(true); 
            } else { moveTokenStepByStepRemote(color, tokenIndex, diceVal); }
        });
...
function showMyIdentity(color) {
    ...
    document.querySelectorAll('.player-profile').forEach(p => {
        p.style.border = "1px solid rgba(255, 255, 255, 0.1)"; p.style.boxShadow = "none"; p.style.opacity = "0.3";
    });
    const myProfile = document.getElementById(`profile-${color}`);
    if (myProfile) {
        myProfile.style.border = `2px solid ${bgColor}`; myProfile.style.boxShadow = `0 0 15px ${bgColor}`; myProfile.style.opacity = "1";
    }
}

function initGameSession() {
    winnersList = []; totalPlayersInGame = activePlayers.length; 
    activePlayers.forEach(c => document.getElementById(`profile-${c}`).style.opacity = "1");
    activePlayers.forEach(c => missedTurns[c] = 0);
    currentPlayerIndex = 0; gameState = 'WAITING_FOR_ROLL'; isMoving = false;
    updateTurnText(); spawnTokens(); startTurnTimer(); 
}

function handlePlayerWin(playerColor) {
    if (!winnersList.includes(playerColor)) {
        winnersList.push(playerColor);
        let rankText = winnersList.length === 1 ? "1st 🥇" : winnersList.length === 2 ? "2nd 🥈" : "3rd 🥉";
        let profileEl = document.getElementById(`profile-${playerColor}`);
        if (profileEl) {
            let rankDiv = document.createElement("div");
            rankDiv.style.color = "#FFD700"; rankDiv.style.fontWeight = "bold"; rankDiv.innerText = rankText;
            profileEl.appendChild(rankDiv);
        }
        if (socket && window.currentRoomId) socket.emit("player-finished", { color: playerColor, rank: rankText });
        activePlayers = activePlayers.filter(c => c !== playerColor);
        if (winnersList.length >= totalPlayersInGame - 1) endMatchAndGoToMenu();
    }
}
...
function endMatchAndGoToMenu() {
    ...
    triggerInterstitialAd("Match Finished", function() {
        ...
        winnersList.forEach((color, index) => { winMessage += `Rank ${index + 1}: ${color.toUpperCase()} \n`; });
        if (activePlayers.length > 0) winMessage += `Eliminated: ${activePlayers[0].toUpperCase()}\n`;
        alert(winMessage);
...
function startTurnTimer() {
    clearTurnTimer(); timeLeft = 25; updateTimerUI();
...
function handleTurnTimeout(color) {
    ...
    if (missedTurns[color] >= 3) {
        clearTurnTimer(); alert(`🚨 ${color.toUpperCase()} missed 3 turns, eliminated from the game!`);
        if (allTokens[color]) allTokens[color].forEach(t => { if (t.element && t.element.parentNode) t.element.parentNode.removeChild(t.element); });
        ...
        updateTurnText(); gameState = 'WAITING_FOR_ROLL'; startTurnTimer(); return;
    }
...
function createBoard() {
    ...
    createBase(board, 'red-base', 'red'); createBase(board, 'green-base', 'green');
    createBase(board, 'blue-base', 'blue'); createBase(board, 'yellow-base', 'yellow');
    for (let r = 0; r < 15; r++) {
        for (let c = 0; c < 15; c++) {
            if ((r < 6 && c < 6) || (r < 6 && c > 8) || (r > 8 && c < 6) || (r > 8 && c > 8)) continue; 
            const cell = document.createElement("div"); cell.classList.add("ludo-cell"); cell.id = `cell-${r}-${c}`; cell.style.gridArea = `${r + 1} / ${c + 1} / span 1 / span 1`;
            ...
            if (safeZones.some(zone => zone.r === r && zone.c === c)) {
                cell.style.backgroundColor = "#e0e0e0"; let star = document.createElement("span"); star.className = "safe-zone-icon"; star.innerHTML = "⭐"; cell.appendChild(star);
            }
...
function createBase(board, colorClass, colorId) {
    const base = document.createElement("div"); base.classList.add("base", colorClass); base.id = colorId + "-base";
    const innerBox = document.createElement("div"); innerBox.classList.add("inner-base");
    for(let i = 0; i < 4; i++) { const slot = document.createElement("div"); slot.classList.add("token-slot"); slot.id = `${colorId}-slot-${i}`; innerBox.appendChild(slot); }
    base.appendChild(innerBox); board.appendChild(base);
}
function spawnTokens() {
    activePlayers.forEach(color => {
        allTokens[color] = [];
        for (let i = 0; i < 4; i++) {
            let token = document.createElement("div"); token.className = `token token-${color}`; token.id = `token-${color}-${i}`;
...
function rollDice() {
    if (socket && window.currentRoomId) { if (myAssignedColor !== activePlayers[currentPlayerIndex]) return; }
    if (activePlayers.length === 0 || gameState !== 'WAITING_FOR_ROLL' || isMoving) return;
    clearTurnTimer(); gameState = 'ROLLING';
    const diceContainer = document.getElementById("dice-container"); diceContainer.classList.add("rolling");
    setTimeout(() => {
        diceContainer.classList.remove("rolling");
        currentDiceValue = Math.floor(Math.random() * 6) + 1; 
        diceContainer.innerText = diceFaces[currentDiceValue]; diceContainer.style.color = currentDiceValue === 6 ? "#ff2a2a" : "#111";
        gameState = 'WAITING_FOR_MOVE'; startTurnTimer(); 
        if (socket && window.currentRoomId) socket.emit('roll-dice-action', { roomId: window.currentRoomId, diceValue: currentDiceValue, playerIndex: currentPlayerIndex });
        checkAvailableMoves();
    }, 500); 
}
function checkAvailableMoves() {
    let currentPlayerColor = activePlayers[currentPlayerIndex]; let movableTokens = [];
    ...
    if (movableTokens.length === 0) { setTimeout(() => switchTurn(false), 500); } 
    else {
        movableTokens.forEach(idx => allTokens[currentPlayerColor][idx].element.classList.add('highlight-move'));
        if (movableTokens.length === 1) setTimeout(() => handleTokenClick(currentPlayerColor, movableTokens[0]), 300);
    }
}
function handleTokenClick(color, tokenIndex) {
    if (socket && window.currentRoomId) { if (myAssignedColor !== activePlayers[currentPlayerIndex] || color !== myAssignedColor) return; }
    if (gameState !== 'WAITING_FOR_MOVE' || color !== activePlayers[currentPlayerIndex] || isMoving) return;
    clearTurnTimer(); let tokenObj = allTokens[color][tokenIndex];
    if (!tokenObj.element.classList.contains('highlight-move')) return; 
    allTokens[color].forEach(t => t.element.classList.remove('highlight-move'));
    if (socket && window.currentRoomId) socket.emit('move-token-action', { roomId: window.currentRoomId, color: color, tokenIndex: tokenIndex, diceVal: currentDiceValue });
    if (tokenObj.state === 'home' && currentDiceValue === 6) { tokenObj.state = 'active'; tokenObj.pathPosition = 0; updateTokenUI(color, tokenIndex); switchTurn(true); } 
    else { moveTokenStepByStep(color, tokenIndex, currentDiceValue); }
}
function moveTokenStepByStep(color, tokenIndex, stepsToMove) {
    isMoving = true; let tokenObj = allTokens[color][tokenIndex]; let stepsTaken = 0;
    let moveInterval = setInterval(() => {
        stepsTaken++; tokenObj.pathPosition++; updateTokenUI(color, tokenIndex);
        if (stepsTaken >= stepsToMove) {
            clearInterval(moveInterval);
            setTimeout(() => { let cutHappened = checkCapture(color, tokenIndex); isMoving = false; switchTurn(cutHappened || currentDiceValue === 6); }, 300);
        }
    }, 250); 
}
...
function updateTokenUI(color, tokenIndex) {
    let tokenObj = allTokens[color][tokenIndex]; let globalPos = (playersData[color].startOffset + tokenObj.pathPosition) % 52;
    let targetCoords = masterPath[globalPos]; let targetCell = document.getElementById(`cell-${targetCoords.r}-${targetCoords.c}`);
    if (targetCell) { targetCell.appendChild(tokenObj.element); tokenObj.element.classList.add('moving'); setTimeout(() => tokenObj.element.classList.remove('moving'), 200); }
}
function checkCapture(color, tokenIndex) {
    let attacker = allTokens[color][tokenIndex]; let globalPos = (playersData[color].startOffset + attacker.pathPosition) % 52;
    let targetCoords = masterPath[globalPos]; if (safeZones.some(zone => zone.r === targetCoords.r && zone.c === targetCoords.c)) return false; 
    ...
    for (let enemyColor of activePlayers) {
        ...
        allTokens[enemyColor].forEach((enemyToken, enemyIndex) => {
            if (enemyToken.state === 'active') {
                if ((playersData[enemyColor].startOffset + enemyToken.pathPosition) % 52 === globalPos) {
                    cutHappened = true; enemyToken.state = 'home'; enemyToken.pathPosition = -1;
...
function switchTurn(gotExtraTurn) {
    clearTurnTimer(); if (!gotExtraTurn) currentPlayerIndex = (currentPlayerIndex + 1) % activePlayers.length;
    updateTurnText(); gameState = 'WAITING_FOR_ROLL'; startTurnTimer(); 
}
function updateTurnText() {
    let currentPlayerColor = activePlayers[currentPlayerIndex];
    const turnText = document.getElementById("turn-text");
    turnText.innerText = playersData[currentPlayerColor].name; turnText.className = playersData[currentPlayerColor].class;
}
